...
const DeleteVillaModalComponent = ({show, handleToggleModal, objectDelete, reloadAfterDelete}) => {
    const navigate = useNavigate();
    const handleDeleteVilla = () => {
        const fetchData = async () => {
            const isDeleteSucceed = await deleteVilla(objectDelete?.id);
            if (isDeleteSucceed) {
    ...
    return (
        <Modal show={show} onHide={handleToggleModal} centered>
            ...
            <Modal.Footer className="border-0 d-flex justify-content-between">
                ...
                <Button variant="danger" className="px-4" onClick={handleDeleteVilla}>
                    Xóa
                </Button>
            </Modal.Footer>
        </Modal>
...
export default DeleteVillaModalComponent;
